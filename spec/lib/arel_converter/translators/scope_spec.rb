  ...
  context 'parsing scopes' do
    ...
    context 'with conditions' do
      ...
      it 'where there is a hash including an array' do
        ...
        expect(ArelConverter::Translator::Scope.translate(scope)).to eq(%Q{scope :receivable, where( state: ["confirmed", "partially_received", "ordered"] )})
      end
      ...
      it 'where there is a where and include' do
        ...
        expect(ArelConverter::Translator::Scope.translate(scope)).to eq(%Q{scope :with_open, includes(:purchase_order).where(["purchase_orders.state NOT IN ('shopping', 'received', 'cancelled', 'closed')"])})
      end
    end
    ...
    context "with lambdas" do
      it 'should stay on a single line' do
        ...
        expect(ArelConverter::Translator::Scope.translate(scope)).to eq(%Q{scope :for_vendor, lambda { |vendor| includes(:vendor_purchase_order).where(["purchase_orders.vendor_id = ?", vendor.id]) }})
      end
    end
    ...
    context "with multiple options" do
      it 'should concatinate them correctly' do
        ...
        expect(ArelConverter::Translator::Scope.translate(scope)).to eq(%Q{scope :receivable, includes([:vendor]).where("state IN ('confirmed','partially_received')")})
      end
    end
